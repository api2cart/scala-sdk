...
object Example extends App {
    ...
    val miraklApiKey: String = a1b2c3d4-e5f6-7890-abcd-ef1234567890 // String | Mirakl API Key
    ...
    val nopcommerceToken: String = igse8e4rdmzkxdi937qe69d59en1imw // String | Access token authorizing the app to access resources on behalf of a user

    val nopcommerceSecret: String = a1f3c9e72b6d480fa5c81e9b7d2f04a63e8c5b1a9d7f3026e4b8c1a2f5d6e093 // String | nopCommerce webhook signing secret generated by the Api2Cart.Connector plugin. Optional when connecting a store, but required to receive webhooks from it.
    ...
    val walmartRefreshToken: String = eyJraWQiOiIxYmIxMTYyNS1lM2Y2LTQyNWMtOTkwMS0zZTdiNmJhNzZmNjIiLCJhbGciOiJSUzI1NiJ9 // String | Walmart refresh token received by a Solution Provider application through the Walmart App Store authorization. When it is set, walmart_client_id and walmart_client_secret are the credentials of that application, not of the seller. Must be used together with walmart_seller_id and is supported only for walmart_region = 'us'

    val walmartSellerId: String = 10000004372 // String | Walmart seller id the refresh token was issued for, sent as the WM_PARTNER.ID header. Must be used together with walmart_refresh_token and is supported only for walmart_region = 'us'
